...
Вычисляется расстояние от выпуклой оболочки до заданного заполненного треугольника.
Треугольник задаётся его вершинами — тремя произвольными точками плоскости.

## Идея модификации

Для каждого из классов `Point`, `Segment`, `Polygon` был реализован метод `convex_dist`.

В классе `Point` сначала с помощью метода `inside_triangle` вычисляется, лежит ли точка
внутри треугольника. Если да, то искомое расстояние равно 0.0 (т.к. треугольник заполненный).
Иначе в методе `point_dist` находится минимальное расстояние до заданного треугольника как 
минимум расстояний до каждого из его рёбер.

В классе `Segment` сначала определяется, лежит ли одна из точек отрезка внутри треугольника и
пересекает ли отрезок одну из сторон треугольника. В обоих случаях искомая величина равна 0.0.
Иначе с помощью метода `dist_between_segments` находится расстояние как минимум из расстояний до
каждого ребра треугольника.

В классе `Polygon` создается словарь, в котором ключами являются пары точек, задающие ребра выпуклой
оболочки, а ключами -- расстояния от этих ребер до треугольника. Это было сделано для удобства перевычисления искомой величины в случае удаления ребер, расстояние от которых до треугольника минимально,
или в случае добавления новых ребер, находящихся на меньшем расстоянии.
